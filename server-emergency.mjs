...
const configuredMaxZoom = Number(process.env.OCCUMED_EMERGENCY_MAX_ZOOM || 5);
...
if (!Number.isSafeInteger(maxZoom) || maxZoom < 0) {
  throw new Error('The emergency overview PMTiles archive has an invalid maximum zoom.');
}
    ...
    try {
      ...
      if (request.headers['if-none-match'] === etag) {
        write(response, 304, Buffer.alloc(0), contentTypes['.pbf'], method, 'public, max-age=3600', {
          ETag: etag,
          'X-Occumed-Tileset': 'emergency-overview-only-v1'
        });
        return;
      }
...
function shutdown(signal) {
  ...
  shuttingDown = true;
  console.log(`Emergency overview server received ${signal}; draining connections.`);
  server.close(() => {
...
server.listen(port, host, () => {
  console.log(`Occu-Med emergency overview server listening on ${host}:${port}.`);
  console.log(`Serving immutable overview PMTiles through zoom ${maxZoom}; Neon and runtime tile synthesis are disabled.`);
});
