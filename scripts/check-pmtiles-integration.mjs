...
const [pkg, helper, router, sourcePass, profile, planner, manifestBuilder, server, workflow, action] = await Promise.all([
...
console.log('PMTiles integration validated: protocol registration, worldwide routing, release proxying, shard builds, manifest publication, and Occu-Med Planetiler schema are present.');
