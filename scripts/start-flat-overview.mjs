...
const maxAssetBytes = Number(process.env.OCCUMED_FLAT_ASSET_MAX_BYTES || 512 * 1024 * 1024);
...
async function localizeArchive() {
  ...
  if (!['http:', 'https:'].includes(parsed.protocol)) {
    throw new Error('Flat overview source must use HTTP or HTTPS.');
  }
  ...
  try {
    const response = await fetch(parsed, {
      ...
      headers: { 'User-Agent': 'Occu-Med-Map/flat-overview-localizer' }
    });
    if (!response.ok || !response.body) {
      throw new Error(`Flat overview archive returned HTTP ${response.status}.`);
    }

    const declaredLength = Number(response.headers.get('content-length'));
    if (Number.isFinite(declaredLength) && declaredLength > maxAssetBytes) {
      throw new Error(`Flat overview archive exceeds ${maxAssetBytes} bytes.`);
    }
    ...
    if (!(await validArchive(temporary))) {
      throw new Error('Downloaded flat overview archive failed PMTiles validation.');
    }
...
try {
  ...
  console.error(`Unable to prepare flat overview archive: ${error.message}`);
  process.exit(1);
}
